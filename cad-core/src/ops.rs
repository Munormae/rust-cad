// cad-core/src/ops.rs
use crate::{Document, Pt2, Entity, EntityKind};
...
pub fn snap_to_grid(p: Pt2, step: f32) -> Pt2 {
    ...
    Pt2 { x: (p.x / step).round() * step, y: (p.y / step).round() * step }
}

pub fn make_line(doc: &mut Document, a: Pt2, b: Pt2, layer: &str) -> u64 {
    doc.add_entity(Entity { id: 0, layer: layer.into(), kind: EntityKind::LineSeg { a, b } })
}

pub fn make_arc(doc: &mut Document, center: Pt2, radius: f32, start: f32, end: f32, layer: &str) -> u64 {
    doc.add_entity(Entity {
        id: 0,
        layer: layer.into(),
        kind: EntityKind::Arc { center, radius, start_angle: start, end_angle: end }
    })
}

pub fn make_polyline(doc: &mut Document, pts: Vec<Pt2>, closed: bool, layer: &str) -> Result<u64> {
    if pts.len() < 2 { return Err(anyhow!("Polyline requires at least 2 points")); }
    Ok(doc.add_entity(Entity { id: 0, layer: layer.into(), kind: EntityKind::Polyline { pts, closed } }))
}
...
pub fn make_nurbs_open_uniform(
    ...
    if degree < 1 { return Err(anyhow!("degree must be >= 1")); }
    let n = ctrl_pts.len();
    if n < degree + 1 { return Err(anyhow!("ctrl_pts must have at least degree + 1 points")); }
    if let Some(w) = &weights {
        if w.len() != n { return Err(anyhow!("weights length must match ctrl_pts length")); }
    }
    ...
    Ok(doc.add_entity(Entity {
        id: 0,
        layer: layer.into(),
        kind: EntityKind::NurbsCurve2D { degree, knots, ctrl_pts, weights },
    }))
}

pub fn nurbs_from_polyline(doc: &mut Document, poly: &[Pt2], layer: &str) -> Result<u64> {
    if poly.len() < 3 { return Err(anyhow!("Need at least 3 points to build degree-2 BSpline")); }
    make_nurbs_open_uniform(doc, 2, poly.to_vec(), None, layer)
}

/// Утилита для добавления текста (опционально)
pub fn make_text(doc: &mut Document, pos: Pt2, content: impl Into<String>, height: f32, layer: &str) -> u64 {
    doc.add_entity(Entity {
        id: 0,
        layer: layer.into(),
        kind: EntityKind::Text { pos, content: content.into(), height },
    })
}
...
pub fn sample_entity_nurbs(ent: &Entity, samples: usize) -> Option<Vec<Pt2>> {
    match &ent.kind {
        EntityKind::NurbsCurve2D { knots, ctrl_pts, .. } => {
...
pub fn translate_entity(ent: &mut Entity, dx: f32, dy: f32) {
    fn shift(p: &mut Pt2, dx: f32, dy: f32) { p.x += dx; p.y += dy; }

    match &mut ent.kind {
        EntityKind::LineSeg { a, b } => { shift(a, dx, dy); shift(b, dx, dy); }
        EntityKind::Arc { center, .. } => { shift(center, dx, dy); }
        EntityKind::Polyline { pts, .. } => { for p in pts { shift(p, dx, dy); } }
        EntityKind::NurbsCurve2D { ctrl_pts, .. } => { for p in ctrl_pts { shift(p, dx, dy); } }
        EntityKind::Text { pos, .. } => { shift(pos, dx, dy); } // ← добавлено
    }
}
