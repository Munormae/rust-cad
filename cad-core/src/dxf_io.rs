...
fn p2(x: f64, y: f64) -> Pt2 {
    Pt2 { x: x as f32, y: y as f32 }
}

#[inline]
fn dpoint(p: Pt2) -> DPoint {
    DPoint { x: p.x as f64, y: p.y as f64, z: 0.0 }
}
    ...
    for e in drawing.entities() {
        ...
        match &e.specific {
            EntityType::Line(line) => {
                ...
                doc.add_entity(Entity { id: 0, layer, kind: EntityKind::LineSeg { a, b } });
            }
            EntityType::LwPolyline(pl) => {
                let pts: Vec<Pt2> = pl.vertices.iter().map(|v| p2(v.x, v.y)).collect();
                let closed = pl.is_closed();
                if pts.len() >= 2 {
                    doc.add_entity(Entity { id: 0, layer, kind: EntityKind::Polyline { pts, closed } });
                }
            }
            EntityType::Spline(sp) => {
                ...
                let weights = if sp.weight_values.is_empty() { None } else { Some(sp.weight_values.clone()) };
            ...
            EntityType::Text(t) => {
                ...
                let height = if t.text_height > 0.0 { t.text_height as f32 } else { 2.5 };
                let content = t.value.clone();
                doc.add_entity(Entity {
                    id: 0, layer,
                    kind: EntityKind::Text { pos, content, height },
                });
            }
            EntityType::MText(mt) => {
                let pos = p2(mt.insertion_point.x, mt.insertion_point.y);
                // ключевое поле: initial_text_height
                let height = if mt.initial_text_height > 0.0 { mt.initial_text_height as f32 } else { 2.5 };
    ...
    for ent in &doc.entities {
        match &ent.kind {
            ...
            EntityKind::Polyline { pts, closed } => {
                ...
                pl.vertices = pts.iter().enumerate().map(|(i, p)| {
            ...
            EntityKind::NurbsCurve2D { degree, knots, ctrl_pts, weights } => {
                ...
                if let Some(w) = weights { sp.weight_values = w.clone(); sp.set_is_rational(true); }
                else { sp.weight_values.clear(); sp.set_is_rational(false); }
