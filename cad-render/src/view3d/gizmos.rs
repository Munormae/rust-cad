...
pub fn draw_origin_axes(cam: &Camera, rect: egui::Rect, painter: &egui::Painter) {
    let axes = [
        (Pt3::new(0.0,0.0,0.0), Pt3::new(1000.0,0.0,0.0), Color32::RED,   "X"),
        (Pt3::new(0.0,0.0,0.0), Pt3::new(0.0,1000.0,0.0), Color32::GREEN, "Y"),
        (Pt3::new(0.0,0.0,0.0), Pt3::new(0.0,0.0,1000.0), Color32::BLUE,  "Z"),
    ];
    for (a,b,color,label) in axes {
        if let (Some(pa), Some(pb)) = (cam.world_to_screen(rect, a), cam.world_to_screen(rect, b)) {
            ...
            let left  = Pos2::new(pb.x - 8.0*dir.x + 4.0*dir.y, pb.y - 8.0*dir.y - 4.0*dir.x);
            let right = Pos2::new(pb.x - 8.0*dir.x - 4.0*dir.y, pb.y - 8.0*dir.y + 4.0*dir.x);
            painter.line_segment([left, pb],  Stroke { width: 1.5, color });
            painter.line_segment([right, pb], Stroke { width: 1.5, color });
            painter.text(pb + 6.0*dir, Align2::LEFT_CENTER, label, FontId::monospace(10.0), color);
        }
    }
}
