...
impl Selection {
    #[inline] pub fn clear(&mut self) { self.ids.clear(); }
    #[inline] pub fn is_empty(&self) -> bool { self.ids.is_empty() }
    #[inline] pub fn add(&mut self, id: u64) { self.ids.insert(id); }
    #[inline] pub fn remove(&mut self, id: u64) { self.ids.remove(&id); }
    #[inline]
    pub fn toggle(&mut self, id: u64) {
        if !self.ids.insert(id) { self.ids.remove(&id); }
    }
...
impl SelectionRect {
    #[inline]
    pub fn start(p: Pt2) -> Self { Self { start: p, current: p } }

    #[inline]
    pub fn update_current(&mut self, p: Pt2) { self.current = p; }

    /// Мировые границы и флаг crossing (true, если тянем справа-налево).
    #[inline]
    pub fn world_bounds(&self) -> (Pt2, Pt2, bool) {
        let min = Pt2 { x: self.start.x.min(self.current.x), y: self.start.y.min(self.current.y) };
        let max = Pt2 { x: self.start.x.max(self.current.x), y: self.start.y.max(self.current.y) };
    ...
    #[inline]
    pub fn screen_bounds(&self, app: &AppState, rect: egui::Rect) -> (egui::Pos2, egui::Pos2, bool) {
