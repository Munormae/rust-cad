...
pub fn derive_bounded_curve(input: TokenStream) -> TokenStream {
    ...
    let trait_name = quote! { cryxtal_geotrait::BoundedCurve };
    let ty = input.ident;
        ...
        Data::Enum(DataEnum { ref variants, .. }) => {
            ...
            quote! {
                ...
                where
                    ...
                    Self: cryxtal_geotrait::ParametricCurve<Point = <#top_ty as ParametricCurve>::Point>, {
                    #(#methods)*
                }
            }
        }
        Data::Struct(DataStruct { ref fields, .. }) => {
            ...
            quote! {
                ...
                where
                    ...
                    Self: cryxtal_geotrait::ParametricCurve<Point
                        = <#field_type as cryxtal_geotrait::ParametricCurve>::Point>, {
...
pub fn derive_bounded_surface(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::BoundedSurface };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_cut(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::Cut };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_invertible(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::Invertible };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_parameter_division_1d(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::ParameterDivision1D };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_parameter_division_2d(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::ParameterDivision2D };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_parametric_curve(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::ParametricCurve };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
        Data::Enum(DataEnum { ref variants, .. }) => {
            ...
            let methods = methods!(
                ...
                fn parameter_range(&self,) -> cryxtal_geotrait::ParameterRange,
...
pub fn derive_parametric_surface(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::ParametricSurface };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
        Data::Enum(DataEnum { ref variants, .. }) => {
            ...
            let methods = methods!(
                ...
                fn parameter_range(&self,) -> (cryxtal_geotrait::ParameterRange, cryxtal_geotrait::ParameterRange),
...
pub fn derive_parametric_surface3d(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name0 = quote! { cryxtal_geotrait::ParametricSurface };
    let trait_name1 = quote! { cryxtal_geotrait::ParametricSurface3D };
    let ty = input.ident;
    let gen = input.generics;
...
pub fn derive_snp_d1(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::SearchNearestParameter::<D1> };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_snp_d2(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::SearchNearestParameter::<D2> };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_sp_d1(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::SearchParameter::<D1> };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_sp_d2(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::SearchParameter::<D2> };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_transform_m3(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::Transformed::<Matrix3> };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
        Data::Enum(DataEnum { ref variants, .. }) => {
            ...
            let methods = methods!(
                variants,
                trait_name,
                fn transform_by(&mut self, mat: cryxtal_geometry::prelude::Matrix3) -> (),
                fn transformed(&self, mat: cryxtal_geometry::prelude::Matrix3) -> Self,
            );
            quote! {
                #[automatically_derived]
                impl #gen #trait_name for #ty #gen
...
pub fn derive_transform_m4(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_geotrait::Transformed::<Matrix4> };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
        Data::Enum(DataEnum { ref variants, .. }) => {
            ...
            let methods = methods!(
                variants,
                trait_name,
                fn transform_by(&mut self, mat: cryxtal_geometry::prelude::Matrix4) -> (),
                fn transformed(&self, mat: cryxtal_geometry::prelude::Matrix4) -> Self,
            );
            quote! {
                #[automatically_derived]
                impl #gen #trait_name for #ty #gen
...
pub fn derive_step_length(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_stepio::out::StepLength };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_display_by_step(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_stepio::out::DisplayByStep };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_step_curve(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_stepio::out::StepCurve };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
...
pub fn derive_step_surface(input: TokenStream) -> TokenStream {
    let input = parse_macro_input!(input as DeriveInput);
    let trait_name = quote! { cryxtal_stepio::out::StepSurface };
    let ty = input.ident;
    let gen = input.generics;
    let where_predicates = gen.where_clause.iter().flat_map(|x| &x.predicates);
    match input.data {
