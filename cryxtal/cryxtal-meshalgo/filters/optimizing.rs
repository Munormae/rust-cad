...
/// Mesh optimization helpers (dedup attributes, remove degenerates, cluster similar attributes).
pub trait OptimizingFilter {
    /// Remove unused positions/uvs/normals and reindex faces.
    fn remove_unused_attrs(&mut self) -> &mut Self;
    /// Remove degenerate faces and fix quads with collapsed edges.
    fn remove_degenerate_faces(&mut self) -> &mut Self;
    /// Cluster nearly identical attributes within tolerance and reindex faces.
