...
/// Normal vector utilities for polygon meshes.
pub trait NormalFilters {
    /// Normalize all existing normals and detach invalid ones from faces.
    fn normalize_normals(&mut self) -> &mut Self;
    /// Assign per-face normals; overwrite existing when requested.
    fn add_naive_normals(&mut self, overwrite: bool) -> &mut Self;
    /// Cluster normals and assign smoothed normals per cluster.
    fn add_smooth_normals(&mut self, tol_ang: f64, overwrite: bool) -> &mut Self;
    /// Flip face winding to match existing normals.
    fn make_face_compatible_to_normal(&mut self) -> &mut Self;
    /// Flip normals to match face orientation where necessary.
