...
/// Collision analysis utilities on polygon meshes.
pub trait Collision {
    /// Returns one pair of intersection points if meshes collide.
    fn collide_with(&self, other: &PolygonMesh) -> Option<(Point3, Point3)>;
    /// Returns all intersecting segment endpoints between two meshes.
