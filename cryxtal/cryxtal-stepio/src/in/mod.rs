...
impl<'a, P> From<&'a Line> for cryxtal_geometry::prelude::Line<P>
where
...
fn uniform_knots(num_ctrl: usize, degree: usize) -> cryxtal_geometry::Result<KnotVec> {
    KnotVec::try_from(
        (0..degree + num_ctrl + 1)
...
impl EdgeCurve {
    ...
    fn sub_parse_2d(
        ...
        // orientation for 2D handled via swapping points already; do not invert enum directly
        Ok(curve)
    }
    ...
    fn sub_parse_curve3d(
        ...
        // orientation for 3D handled by swapping endpoints in the caller; do not invert enum directly
        Ok(curve)
    }
}
        ...
        shell
            ...
            .filter_map(|(orientation, face)| {
                ...
                let mut surface = Surface::try_from(&step_surface)
                    .map_err(|e| eprintln!("{e}"))
                    .ok()?;
                // orientation handled later in topology conversion; avoid relying on Invertible here
