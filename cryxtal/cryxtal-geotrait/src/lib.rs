...
//! Geometric traits for **CryXtal**.
//!
//! - Полностью замена `truck-*` → `cryxtal-*`.
//! - Для совместимости с дерив-макросами оставлен скрытый алиас
//!   `::truck_geotrait` на текущий крейт. Это не меняет публичный API.

#[doc(hidden)]
pub extern crate self as truck_geotrait; // нужно дерив-макросам, которые ждут ::truck_geotrait

/// Паника, если допуск меньше минимально допустимого.
#[macro_export]
#[doc(hidden)]
