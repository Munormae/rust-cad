...
impl<P: ControlPoint<f64>> ParametricSurface for BSplineSurface<P> {
    ...
    fn parameter_range(&self) -> (ParameterRange, ParameterRange) {
        BSplineSurface::parameter_range(self)
    }
}
...
impl IncludeCurve<BSplineCurve<Point2>> for BSplineSurface<Point2> {
    fn include(&self, curve: &BSplineCurve<Point2>) -> bool {
        let pt = curve.front();
        let mut hint = surface::presearch(self, pt, self.range_tuple(), PRESEARCH_DIVISION);
        hint = match surface::search_parameter(self, pt, hint, INCLUDE_CURVE_TRIALS) {
            Some(got) => got,
            None => return false,
        };
