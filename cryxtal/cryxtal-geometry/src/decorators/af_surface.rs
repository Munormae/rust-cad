...
mod subders {
    use cryxtal_base::cgmath64::{Vector2, Vector3, Vector4};
    use cryxtal_base::ders::{CurveDers, SurfaceDers};
...
use proptest::{prelude::*, property_test};
    ...
    fn test_ders(#[strategy = 0.0..=1.0] u: f64, #[strategy = 0.0..=1.0] v: f64) {
        ...
        cryxtal_base::assert_near!(surface.normal(0.0, v), surface0.normal(u0, v0));

        let pt1 = surface.subs(1.0, v);
        let (u1, v1) = surface1.search_parameter(pt1, (0.5, 0.5), 100).unwrap();
        cryxtal_base::assert_near!(surface.normal(1.0, v), surface1.normal(u1, v1));
    }
}
