...
mod double_projection_tests {
    ...
    fn exec_plane_case(c0: [f64; 3], n0: [f64; 2], c1: [f64; 3], n1: [f64; 2]) -> PResult {
        ...
        for i in 0..10 {
            ...
            cryxtal_base::prop_assert_near!(q, plane0.subs(p0.x, p0.y));
            cryxtal_base::prop_assert_near!(q, plane1.subs(p1.x, p1.y));
            if let Some(o) = o {
                cryxtal_base::prop_assert_near!(q.distance2(o), t * t);
            } else {
                o = Some(q);
            }
        }
    ...
    fn exec_sphere_case(t: f64, r: f64) -> PResult {
        ...
        cryxtal_base::prop_assert_near!(q, sphere0.subs(p0.x, p0.y));
        cryxtal_base::prop_assert_near!(q, sphere1.subs(p1.x, p1.y));
        cryxtal_base::prop_assert_near!(q, Point3::new(f64::cos(t), f64::sin(t), 0.0));
        Ok(())
    }
