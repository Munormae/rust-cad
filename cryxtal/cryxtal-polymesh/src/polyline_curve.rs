...
fn polyline_test() {
    ...
    cryxtal_base::assert_near!(polyline.subs(2.5), Point3::new(0.0, 0.5, 0.5));

    let mut part0 = polyline.clone();
    let part1 = part0.cut(2.5);
    assert_eq!(part0.len(), 4);
    assert_eq!(part1.len(), 6);
    cryxtal_base::assert_near!(part0.subs(2.5), Point3::new(0.0, 0.75, 0.25));
    cryxtal_base::assert_near!(part1.subs(0.5), Point3::new(0.0, 0.25, 0.75));
    let mut part0 = polyline.clone();
    let part1 = part0.cut(4.0);
    assert_eq!(part0.len(), 5);
    assert_eq!(part1.len(), 4);
    cryxtal_base::assert_near!(part0.subs(2.5), Point3::new(0.0, 0.5, 0.5));
    cryxtal_base::assert_near!(part1.subs(0.5), Point3::new(0.5, 0.5, 1.0));

    let pt = polyline.subs(2.13);
    let t = polyline.search_parameter(pt, None, 1).unwrap();
    cryxtal_base::assert_near!(t, 2.13);

    let pt = Point3::new(2.0, 0.0, 0.0);
    assert!(polyline.search_parameter(pt, None, 1).is_none());
    cryxtal_base::assert_near!(polyline.search_nearest_parameter(pt, None, 1).unwrap(), 1.0);
    ...
    cryxtal_base::assert_near!(div.1[0], Point3::new(0.5, 0.5, 0.0));
    cryxtal_base::assert_near!(div.1[1], Point3::new(0.0, 1.0, 0.0));
    cryxtal_base::assert_near!(div.1[2], Point3::new(0.0, 0.0, 1.0));
    cryxtal_base::assert_near!(div.1[3], Point3::new(0.0, 1.0, 1.0));
    cryxtal_base::assert_near!(div.1[4], Point3::new(1.0, 0.0, 1.0));
    cryxtal_base::assert_near!(div.1[5], Point3::new(1.0, 1.0, 0.0));
    cryxtal_base::assert_near!(div.1[6], Point3::new(1.0, 1.0, 0.2));
}
